...
const validateUserRegistration = [
  ...
  body('agreePrivacy').equals('true').withMessage('개인정보처리방침에 동의해야 합니다'),
...
const validateValuesAssessment = [
  body('answers').isObject().withMessage('답변은 객체 형태여야 합니다'),

  body('answers.*').isObject().withMessage('각 답변은 객체 형태여야 합니다'),

  body('answers.*.questionId').isInt({ min: 1, max: 100 }).withMessage('유효한 질문 ID여야 합니다'),

  body('answers.*.value').notEmpty().withMessage('답변 값은 필수입니다'),

  body('answers.*.category').optional().isString().withMessage('카테고리는 문자열이어야 합니다'),
...
const validateMessageSend = [
  ...
  body('replyTo').optional().isMongoId().withMessage('유효한 메시지 ID여야 합니다'),
...
const validatePagination = [
  ...
  query('sort').optional().isString().withMessage('정렬 기준은 문자열이어야 합니다'),
...
const validateObjectId = (paramName = 'id') => [
  param(paramName).isMongoId().withMessage('유효한 ID가 아닙니다'),
