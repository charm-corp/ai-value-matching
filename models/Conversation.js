...
conversationSchema.methods.getUnreadCount = function (userId) {
  const readStatus = this.readStatus.find(rs => rs.userId.toString() === userId.toString());
...
conversationSchema.methods.incrementUnreadCount = function (userId) {
  ...
  this.participants.forEach(participantId => {
    if (participantId.toString() !== userId.toString()) {
      const readStatus = this.readStatus.find(
        rs => rs.userId.toString() === participantId.toString()
      );
...
conversationSchema.methods.updateTypingStatus = function (userId, isTyping) {
  if (isTyping) {
    ...
    this.typingUsers = this.typingUsers.filter(tu => tu.userId.toString() !== userId.toString());
...
conversationSchema.methods.updateLastMessage = function (message) {
  ...
  // 발신자의 메시지 수 증가
  const userStats = this.stats.messagesByUser.find(
    mbu => mbu.userId.toString() === message.sender.toString()
  );
...
conversationSchema.statics.findByMatch = function (matchId) {
  return this.findOne({ matchId: matchId }).populate(
    'participants',
    'name profileImage lastActive'
  );
};
