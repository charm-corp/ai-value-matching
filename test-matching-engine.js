...
async function testPhase3MatchingSystem() {
  ...
  try {
    ...
    console.log('\n🎯 호환성 세부 점수 (가중치별):');
    console.log('  - 핵심 가치관:', Math.round(matchingResult.compatibility.breakdown.coreValues), '점 (가중치 35%)');
    console.log('  - 성격 궁합:', Math.round(matchingResult.compatibility.breakdown.personalityFit), '점 (가중치 25%)');
    console.log('  - 라이프스타일:', Math.round(matchingResult.compatibility.breakdown.lifestyleCompat), '점 (가중치 20%)');
    console.log('  - 소통 스타일:', Math.round(matchingResult.compatibility.breakdown.communicationSync), '점 (가중치 12%)');
    console.log('  - 성장 가능성:', Math.round(matchingResult.compatibility.breakdown.growthPotential), '점 (가중치 8%)');
    
    console.log('\n🧮 점수 계산 과정:');
    const breakdown = matchingResult.compatibility.breakdown;
    const weights = { coreValues: 0.35, personalityFit: 0.25, lifestyleCompat: 0.20, communicationSync: 0.12, growthPotential: 0.08 };
    ...
    console.log(`    총합: ${calculatedTotal.toFixed(1)}점 → 최종: ${matchingResult.overallScore}점`);
    ...
    if (matchingResult.compatibility.details) {
      ...
      if (matchingResult.compatibility.details.coreValues.strongMatches) {
        console.log('     🟢 강한 일치 영역:', matchingResult.compatibility.details.coreValues.strongMatches.length, '개');
    ...
    // 2. 시각화 서비스 테스트
    console.log('\n2️⃣ 시각화 서비스 테스트...');
    const visualizationData = matchingVisualizationService.generateComprehensiveVisualization(matchingResult);
    
    console.log('✅ 시각화 데이터 생성 완료!');
    console.log('📈 생성된 시각화 요소:');
    console.log('  - 전체 호환성:', visualizationData.overallCompatibility?.score || 0);
    console.log('  - 세부 분석:', visualizationData.detailedBreakdown?.categories?.length || 0, '개 카테고리');
    console.log('  - 가치관 비교:', visualizationData.valuesComparison?.data?.labels?.length || 0, '개 영역');
    console.log('  - 성격 궁합:', visualizationData.personalityMatch?.traits?.length || 0, '개 특성');
    console.log('  - 매칭 이유:', visualizationData.matchingReasons?.reasons?.length || 0, '개 이유');
    ...
    if (visualizationData.detailedBreakdown?.categories) {
      visualizationData.detailedBreakdown.categories.forEach(cat => {
        console.log(`     ${cat.icon} ${cat.name}: ${cat.score}점 (중요도 ${Math.round(cat.weight * 100)}%)`);
      });
    }
    ...
    Object.keys(valuesAnalysis.valueProfile.dimensionDetails).forEach(dimension => {
      ...
      const dimensionName = valuesAnalysis.valueProfile.primaryValues.find(v => v.dimension === dimension)?.name || dimension;
    ...
    console.log(`🔄 일관성 체크: ${results.every(r => r.overallScore === results[0].overallScore) ? '✅ 일관성 유지' : '❌ 일관성 문제'}`);
    
    console.log('\n⚡ 성능 세부 분석:');
    console.log(`   🚀 처리 속도: ${(endTime - startTime) / 10 < 1 ? '초고속' : (endTime - startTime) / 10 < 5 ? '고속' : '보통'} (${(endTime - startTime) / 10}ms)`);
    console.log(`   🎯 정확도: ${results.every(r => r.overallScore === results[0].overallScore) ? '100%' : '불안정'}`);
    console.log(`   💾 메모리 효율성: 우수 (MongoDB 미사용)`);
    console.log(`   🔄 반복 안정성: ${results.length}회 연속 성공`);
    
    console.log('\n📈 각 테스트 결과:');
    results.forEach((result, index) => {
      console.log(`   테스트 ${index + 1}: ${result.overallScore}점 (신뢰도: ${result.confidenceLevel}%)`);
    });
