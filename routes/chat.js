...
const { authenticate, requireVerified, requireConversationParticipant } = require('../middleware/auth');
const { validateMessageSend, validatePagination, validateObjectId } = require('../middleware/validation');
 ...
 */
router.get('/conversations', authenticate, requireVerified, validatePagination, async (req, res) => {
  try {
 ...
 */
router.get('/conversations/:id', authenticate, validateObjectId('id'), requireConversationParticipant, async (req, res) => {
  try {
 ...
 */
router.get('/conversations/:id/messages', authenticate, validateObjectId('id'), requireConversationParticipant, validatePagination, async (req, res) => {
  try {
 ...
 *         description: 잘못된 요청
 */
router.post('/conversations/:id/messages', authenticate, validateObjectId('id'), requireConversationParticipant, validateMessageSend, async (req, res) => {
  try {
    ...
    if (replyTo) {
      ...
      if (!replyMessage || replyMessage.conversationId.toString() !== conversation._id.toString()) {
        return res.status(400).json({
          success: false,
...
router.post('/conversations/start', authenticate, requireVerified, async (req, res) => {
  try {
    ...
    const isParticipant = match.user1._id.toString() === userId || match.user2._id.toString() === userId;
 ...
 */
router.post('/conversations/:id/typing', authenticate, validateObjectId('id'), requireConversationParticipant, async (req, res) => {
  try {
