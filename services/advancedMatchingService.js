...
class AdvancedMatchingService {
  ...
  async calculateValuesCompatibility(user1, user2) {
    try {
      ...
      Object.keys(values1).forEach(key => {
        if (values1[key] && values2[key] && values1[key] !== 'undefined' && values2[key] !== 'undefined') {
      ...
      Object.keys(personality1).forEach(key => {
        if (personality1[key] && personality2[key] && personality1[key] !== 'undefined' && personality2[key] !== 'undefined') {
      ...
      const avgPersonalitySimilarity = personalityCount > 0 ? personalitySimilarity / personalityCount : 0.5;
  ...
  calculateLifestyleCompatibility(user1, user2) {
    ...
    const lifestyleFactors = [
      'livingArrangement', 'homeOwnership', 'fitnessLevel', 
      'socialLevel', 'travelFrequency'
    ];
    ...
    lifestyleFactors.forEach(factor => {
      if (lifestyle1[factor] && lifestyle2[factor]) {
        if (lifestyle1[factor] === lifestyle2[factor]) {
          ...
          score += this.getLifestyleCompatibilityScore(factor, lifestyle1[factor], lifestyle2[factor]);
        }
        factors++;
      }
    });
